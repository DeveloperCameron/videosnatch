...
function checkYtDlp() {
  return new Promise((resolve) => {
    exec("/opt/homebrew/bin/yt-dlp --version", (err, stdout) => {
      resolve(!err);
    });
  });
}
...
app.get("/api/info", async (req, res) => {
  ...
  exec(
    `/opt/homebrew/bin/yt-dlp --dump-json --no-playlist "${url.replace(/"/g, '\\"')}"`,
    ...
    (err, stdout, stderr) => {
      ...
      try {
        ...
        // Pull out the formats grouped by quality
...
// POST /api/download — start a download, stream progress via SSE
app.get("/api/download", async (req, res) => {
  ...
  const proc = spawn("/opt/homebrew/bin/yt-dlp", args);
  ...
  proc.stdout.on("data", (data) => {
    ...
    for (const line of lines) {
      ...
      if (mergedMatch) filename = mergedMatch[1];
    }
  });
  ...
  proc.on("close", (code) => {
    if (code === 0) {
      ...
      if (finalFile && fs.existsSync(finalFile)) {
        ...
        send({
          type: "done",
          downloadUrl: `/downloads/${basename}`,
          filename: basename,
        });
      } else {
